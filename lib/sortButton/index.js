...
VueComponent({
  ...
  methods: {
    ...
    changeState(value = 0) {
      ...
      if (this.data.oneArrow && value !== -1 && value !== 1) {
      ...
      const {
        oneArrow,
        color
      } = this.data; // 要设置的箭头如果有color，说明其当前处于选中状态，本次行为无效。

      if (this.data[direction] && this.data[direction].color === color) {
        return;
      }
