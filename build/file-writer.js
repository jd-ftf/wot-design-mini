...
const routesConfig = readYaml.sync(path.resolve(__dirname, '../docs/routes.yml'))
const {
  siteMapUrls,
  createSitemapXml
} = require('./sitemap')
...
// 写入 sitemap.xml 文件
const sitemapWriter = () => {
  const originUrl = 'https://ftf.jd.com/wot-design-mini/' + pkg.version + '/#'
  const sitemapUrl = siteMapUrls(routesConfig, originUrl)
  const sitemapXml = createSitemapXml(sitemapUrl)

  // 创建sitemap操作
  const sitemapDir = path.resolve(__dirname, '../docs/')
  const sitemap = path.resolve(__dirname, '../docs/sitemap.xml')
