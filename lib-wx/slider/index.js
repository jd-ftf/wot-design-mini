...
VueComponent({
  ...
  methods: {
    ...
    sliderControl() {
      ...
      const handlePosition = this.checkType(value) === 'Number' ? [this.value2Pos(value), 0] : [this.value2Pos(value[0]), this.value2Pos(value[1])];
      this.setData({
        handlePosition,
    ...
    initState() {
      Promise.all([this.getRect('.be-slider__handle-container'), this.getRect('.be-slider__axle')]).then(rects => {
    ...
    value2Pos(value) {
      ...
      const fixedValue = this.fixValue(value); // 移动距离的宽度比例
