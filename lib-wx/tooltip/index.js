...
VueComponent({
  ...
  methods: {
    ...
    control() {
      ...
      const contentWidth = 10; // 上下位（纵轴）对应的距离左边的距离

      const verticalX = this.width / 2; // 上下位（纵轴）对应的距离底部的距离

      const verticalY = contentWidth + this.height; // 左右位（横轴）对应的距离左边的距离

      const horizontalX = this.width + contentWidth; // 左右位（横轴）对应的距离底部的距离

      const horizontalY = this.height / 2;
      const placements = new Map([// 上
      ['top', `left: ${verticalX}px; bottom: ${verticalY}px; transform: translateX(-50%);`], ['top-start', `left:${0}; bottom: ${verticalY}px;`], ['top-end', `right: ${0}; bottom: ${verticalY}px;`], // 下
      ['bottom', `left: ${verticalX}px; top: ${verticalY}px; transform: translateX(-50%);`], ['bottom-start', `left:${0}; top: ${verticalY}px;`], ['bottom-end', `right: ${0}; top: ${verticalY}px;`], // 左
      ['left', `right: ${horizontalX}px; top: ${horizontalY}px; transform: translateY(-50%);`], ['left-start', `right: ${horizontalX}px; top: ${0};`], ['left-end', `right: ${horizontalX}px; bottom: ${3}px;`], // 右
      ['right', `left: ${horizontalX}px; top: ${horizontalY}px; transform: translateY(-50%);`], ['right-start', `left: ${horizontalX}px; top: ${0};`], ['right-end', `left: ${horizontalX}px; bottom: ${3}px;`]]);
      this.setData({
