...
VueComponent({
  relations: {
    '../radio/index': {
      ...
      linked(child) {
        ...
        const key = child.data.value; // 此radio绑定的value已经被radio占用
  ...
  methods: {
    ...
    renameChild(newName, oldName) {
      if (!this.children || this.children.size === 0) return; // radio不允许绑定重复的value，否则就不是单选框了。此value已经被其它radio绑定。
      ...
      this.children.delete(oldName); // 如果此radio恰巧被radioGroup选中，则把保存选中value的select变量也同步赋值
    ...
    changeSelect(value) {
      ...
      if (value === this.data.value) {
        ...
        const newChild = this.children.get(value); // 如果radio实例存在,并且此radio没被选中,再去选中radio才有意义
        ...
        if (newChild && value !== this.data.select) {
          ...
          if (this.inited) this.$emit('change', value); // 只要选中过radio，inited值永久为true
